...
The current portable subset supports stdio MCP only. Portable Streamable HTTP and legacy SSE entries are reported and skipped because the native remote client does not yet prove the v1 configured-header redirect boundary end to end. Agent Plugins v1 does not define trust, permissions, provenance, or a sandbox. Enabling a package grants its instructions and local executable the same full-trust posture as other installed Hermes plugins.
