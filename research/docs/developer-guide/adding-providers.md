...
Prepare the [independent test environment](/docs/developer-guide/contributing#manual-development-and-test-environment), then use the canonical runner, which isolates each file and scrubs credentials:
...
After tests, run a real smoke test from the checkout using the [PM developer workflow](/docs/reference/package-management#developer-workflow) and its isolated development home. Leave any test venv before PM activation.
