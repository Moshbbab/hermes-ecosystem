...
Tagged `vMAJOR.MINOR.PATCH` releases that passed the stable release gate

`canary`

`hermes-canary`

Prerelease builds from canary tags; versions carry `~canary.<timestamp>`
...
`$PREFIX/bin/hermes`, `$PREFIX/bin/hermes-agent`, `$PREFIX/bin/hermes-acp`

Configuration and user data

`~/.hermes/`, or the selected `HERMES_HOME`
