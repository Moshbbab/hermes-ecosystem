...
If the script reports `missing_dependency`, use `terminal` with a PM-prepared Hermes checkout to prepare the declared `youtube` extra, then reactivate:
...
Follow the isolated development-home setup in [Package Management](https://hermes-agent.nousresearch.com/docs/reference/package-management#developer-workflow) before preparation. Retry `youtube_quiz.py` with that Python and the actual skill directory returned by `skill_view`. For a remote or sandbox terminal, prepare an independent helper environment on that host. Never pip-install into Hermes's selected environment.
