...
Run the setup script with Python from the Hermes environment, not an unrelated system Python. `--install-deps` syncs Hermes' declared Google extra through PM; after syncing, restart Hermes and rerun the OAuth command. If Hermes is not importable, use `hermes setup` first rather than installing packages with pip.
