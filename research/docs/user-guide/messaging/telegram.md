...
For a suspected late replay, compare both occurrences' bot/profile, chat/topic, `update_id`, update kind, `message_id` and actual receive time. An edit can reuse `message_id`, and the message's sent timestamp is not its receive time.
