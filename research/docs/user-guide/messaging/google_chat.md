...
`--install-deps` asks PM to add the `google-chat` extra to the managed Python environment (`pm.sync_venv`); restart the gateway after it finishes so the new environment is active. On Docker / hosted images the venv is read-only and on-demand installs are disabled (`HERMES_DISABLE_LAZY_INSTALLS=1`), so this step cannot add anything there. The published image bakes the `[google-chat]` extra instead, so a fresh container does not need a first-boot install.
