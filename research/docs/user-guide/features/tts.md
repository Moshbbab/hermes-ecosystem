...
**Install via `hermes tools`** → Voice & TTS → Piper. Hermes requests the `piper` extra through PM. From a prepared source checkout, the explicit command is `python -c "import pm; pm.sync_venv(['piper'], explicit=True)"`. Platform markers still apply.
...
Install this external command provider in its own tool environment, not in Hermes's Python environment. Make its executable available on `PATH`.
