...
`--clone-channels` is refused when a running multiplexed gateway already serves the source (the copy would be parked immediately) and otherwise prints a warning naming the platforms now shared with the source. `hermes profile list` prints the same warning for any existing profile whose bot credential is byte-identical to the default's, so older clones surface before they bite. The key set is derived from the platform adapters themselves (registry entries and the gateway's env table), so a newly added platform is covered automatically.
